...
def load_model(model_file_path="", model_name="mn10_as", mode="all", add_se_features=False,
               add_block_features=False, add_classifier_features=False, add_averaged_mels=False):
    model = get_basic_model(model_name=model_name, mode=mode, add_se_features=add_se_features,
                            add_block_features=add_block_features, add_classifier_features=add_classifier_features,
                            add_averaged_mels=add_averaged_mels
                            )
...
def get_basic_model(model_name="mn10_as", **kwargs):
    ...
    model = MNHearWrapper(mel=mel, net=net, **kwargs)
    return model
