...
def load_model(model_file_path="", model_name="mn40_as_ext", mode="logits", add_classifier_features=True):
    model = get_basic_model(model_name=model_name, mode=mode, add_classifier_features=add_classifier_features)
...
def get_basic_model(model_name="mn40_as_ext", **kwargs):
    ...
    model = MNHearWrapper(mel=mel, net=net, scene_embedding_size=5647, timestamp_embedding_size=5647, **kwargs)
    return model
